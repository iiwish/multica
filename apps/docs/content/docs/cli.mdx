...
`daemon status` and `daemon disk-usage` stay available so an agent can inspect the runtime hosting it, scoped to that runtime: `status` probes the health port the daemon injected, and `disk-usage` scans the workspaces root the daemon injected. Neither accepts `--profile`, and `disk-usage` also rejects `--all-profiles` and `--workspaces-root`. Its STATUS and RESUME columns are blank because resolving them would require the human's stored token; retention estimates use the hosting daemon's effective GC policy only when its health endpoint confirms that it owns the scanned root. Use `du` and `df` for anything outside that scope.
...
| | `disk-usage` | View local disk usage and task-environment retention | `--by-workspace`, `--by-task`, `--top`, `--output` |
