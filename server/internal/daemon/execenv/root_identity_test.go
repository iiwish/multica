...
// TestPruneTaskRootRecordsKeepsTheFreezeWindow is the guard that matters most
...
func TestPruneTaskRootRecordsKeepsTheFreezeWindow(t *testing.T) {
	t.Parallel()
	...
	if removed := PruneTaskRootRecords(root, pruneTTL, time.Now(), testLogger()); removed != 0 {
...
func TestPruneTaskRootRecordsReclaimsOnlyStrandedRecords(t *testing.T) {
	t.Parallel()
	...
	if removed := PruneTaskRootRecords(root, pruneTTL, past, testLogger()); removed != 1 {
...
func TestPruneTaskRootRecordsKeepsUnreadableRecords(t *testing.T) {
	t.Parallel()
	...
	past := time.Now().Add(pruneTTL + time.Hour)
	if removed := PruneTaskRootRecords(root, pruneTTL, past, testLogger()); removed != 0 {
...
func TestPruneTaskRootRecordsReclaimsAbandonedStagingDirs(t *testing.T) {
	t.Parallel()
