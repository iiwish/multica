...
`multica daemon disk-usage --by-task` exposes the lifecycle signals behind each retained directory. Table output includes `LOCAL`, `RESUME`, `RETAINED BY`, and `EST. CLEANUP`; JSON includes `task_id`, `completed_at`, `local_directory`, `resume_candidate`, `retention_reason`, `estimated_cleanup_at`, and the running daemon's effective `gc_policy`. `local_directory` is `null` when completion metadata is unavailable rather than guessing `false`. `resume_candidate` is authoritative when the server selects the task's `prior_work_dir` for its `(agent, issue)` scope and the managed workdir still exists locally. It is always `false` for an in-place `local_directory` task root and for a finalized disposable local worktree, because follow-ups provision a fresh environment around the project path instead of reusing that root. Resume state is independent from GC retention: an older directory can have no resume value while the open parent issue still retains it.

Cleanup times are estimates for full task-directory eligibility, not promises of removal and not artifact-only reclamation. Active-use guards and the periodic scan interval can delay deletion after that time. Estimates are calculated from the owning daemon's `/health` policy snapshot rather than the CLI process's `MULTICA_GC_*` environment, and only when the daemon's normalized `workspaces_root` matches the directory that was scanned. A stopped or older daemon, or a daemon still running with a different root after profile configuration changes, produces `policy_unavailable`; an offline or unauthenticated server produces `parent_status_unavailable`; unsupported task kinds produce `kind_policy_unavailable`. Other stable reasons identify disabled GC, user-owned local directories, missing metadata, inaccessible parents, active parents, terminal-parent grace, completed-task grace, or an environment that is already cleanup-eligible. The report remains available in every degraded case and leaves unavailable values blank or `null` instead of guessing.

The command reports the `.repos` footprint on its own line rather than folding it into the per-task totals — every task in a workspace checks out from that shared cache, so attributing it to individual task directories would double-count it. The repo cache is reclaimed on the schedule above and not by any per-issue status change, so it is normal for it to persist after every task directory is gone.
