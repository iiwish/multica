...
Cleanup times are estimates for full task-directory eligibility, not promises of removal and not artifact-only reclamation. Active-use guards and the periodic scan interval can delay deletion after that time. Estimates are calculated from the owning daemon's `/health` policy snapshot rather than the CLI process's `MULTICA_GC_*` environment. A stopped or older daemon produces `policy_unavailable`; an offline or unauthenticated server produces `parent_status_unavailable`; unsupported task kinds produce `kind_policy_unavailable`. Other stable reasons identify disabled GC, user-owned local directories, missing metadata, inaccessible parents, active parents, terminal-parent grace, completed-task grace, or an environment that is already cleanup-eligible. The report remains available in every degraded case and leaves unavailable values blank or `null` instead of guessing.
