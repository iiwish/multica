...
`multica daemon disk-usage --by-task` exposes the lifecycle signals behind each retained directory. Table output includes `LOCAL`, `RESUME`, `RETAINED BY`, and `EST. CLEANUP`; JSON includes `task_id`, `completed_at`, `local_directory`, `resume_candidate`, `retention_reason`, `estimated_cleanup_at`, and the running daemon's effective `gc_policy`. `resume_candidate` is authoritative only when the server can resolve the current resumable `prior_work_dir` for that task's `(agent, issue)` scope. It is independent from GC retention: an older directory can have no resume value while the open parent issue still retains it.
